...
- `index.html` — the standalone, self-contained page (open directly in a browser, or serve as a static site). Single file: inline CSS + JS, no build step, no dependencies. Live at <https://demo.ovk.zero2one.ee/mockup> — the server bakes this file in, so it ships with a normal server redeploy.
...
- Two demo scenarios set in Tallinn (city school network + language transition; diagnostics turnaround at a lab with Tallinn collection points), each with an "Original submission" and "Revised" draft, plus a "Blank/custom" mode with a live keyword-trigger re-scoring engine.
- 8 personas per scenario, segmented by Tallinn district (Kesklinn/Kristiine, Nõmme, Mustamäe, Lasnamäe) alongside the media and watchdog voices — including a hostile-amplification voice whose output is always shown locked (score + flag reason only — never copyable reaction text).
